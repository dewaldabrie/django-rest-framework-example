...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        bulk = Person._get_collection().initialize_ordered_bulk_op()
        ...
        with open(options['filepath']) as f:
            ...
            for p in person_records:
                # add split fields for fruit and veggies
                p['favouriteFruits'], p['favouriteVegetables'] = Person.split_foods_into_fruits_and_vegetables(p['favouriteFood'])
                # parse the datetime
                p['registered'] = parse_datetime(p['registered'].replace(' ',''))
                # parse money balance
                p['balance'] = float(p['balance'].replace('$','').replace(',',''))

                bulk.find({"index": p["index"]}).upsert().replace_one(p)
